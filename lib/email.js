...
function generateICS(booking) {
  ...
  return [
    ...
    `DESCRIPTION:30-minute strategy call with VelosLuxe. We'll walk through your specific revenue leaks and show you how to fix them.${zoomLine}\\n\\nQuestions? Call (740) 301-8119`,
    booking.zoom_link ? `URL:${booking.zoom_link}` : '',
...
async function send(to, subject, html, attachments) {
  ...
  try {
    const opts = { from: FROM, to, subject, html, reply_to: 'Dallaspowers2005@gmail.com' };
...
function getTzAbbr(booking) {
  try {
    const tz = booking.timezone || 'America/Phoenix';
    return new Date(booking.start_time).toLocaleTimeString('en-US', { timeZone: tz, timeZoneName: 'short' }).split(' ').pop();
  } catch { return 'MST'; }
}
...
async function sendBookingConfirmation(booking, qualifying) {
  ...
  const timeStr = date.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', timeZone: booking.timezone || 'America/Phoenix' });
    ...
    <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 560px; margin: 0 auto; color: #1a1a2e;">
      <div style="padding: 40px 32px; background: #ffffff; border-radius: 16px; border: 1px solid #e8e8f0;">
        ...
        <div style="background: #f8f7fc; border-radius: 12px; padding: 20px 24px; margin-bottom: 24px;">
          <table style="width: 100%; border-collapse: collapse;">
            ...
            <tr>
              ...
              <td style="padding: 8px 0; color: #1a1a2e; font-size: 14px; font-weight: 600; text-align: right;">${timeStr} ${getTzAbbr(booking)}</td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td style="padding: 8px 0; color: #1a1a2e; font-size: 14px; font-weight: 600; text-align: right;">${booking.zoom_link ? 'Zoom Video Call' : 'Phone Call'}</td>
            </tr>
            <tr>
        ...
        <div style="text-align: center; margin-bottom: 24px;">
          <a href="${booking.zoom_link}" style="display: inline-block; padding: 16px 48px; background: #2D8CFF; color: #ffffff; border-radius: 100px; text-decoration: none; font-weight: 700; font-size: 16px; letter-spacing: 0.5px;">
            Join Zoom Meeting
          </a>
          <p style="color: #a0a0b0; font-size: 12px; margin-top: 8px;">Save this link — you'll need it on call day</p>
        </div>
        ` : ''}

        <div style="background: #fef9f0; border: 2px solid #c9a46c; border-radius: 12px; padding: 16px 24px; margin-bottom: 24px; text-align: center;">
          <p style="color: #92600e; font-size: 15px; font-weight: 600; margin: 0;">Reply to this email with "Confirmed" to lock in your spot</p>
        </div>
  ...
  const html = `
    <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 560px; margin: 0 auto; color: #1a1a2e;">
      <div style="padding: 40px 32px; background: #ffffff; border-radius: 16px; border: 1px solid #e8e8f0;">
        ...
        <p style="color: #6b6b80; font-size: 15px; line-height: 1.6; margin: 0 0 24px;">
          Quick reminder — your reserved strategy session is <strong>${dateStr} at ${timeStr} ${getTzAbbr(booking)}</strong>.
        </p>

        <div style="background: #1a2e1a; border-radius: 12px; padding: 24px; margin-bottom: 24px;">
          <h3 style="font-size: 14px; color: #6ecf8a; margin: 0 0 12px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase;">Did you know?</h3>
          <p style="color: #e0e0e0; font-size: 15px; line-height: 1.7; margin: 0;">
            The average med spa takes <strong style="color: #ffffff;">42 hours</strong> to respond to a new lead.
        ...
        ${booking.zoom_link ? `
        <div style="text-align: center; margin-bottom: 24px;">
          <a href="${booking.zoom_link}" style="display: inline-block; padding: 14px 40px; background: #2D8CFF; color: #ffffff; border-radius: 100px; text-decoration: none; font-weight: 600; font-size: 15px;">
            Your Zoom Link
          </a>
        </div>
        ` : `
...
async function sendReminder1h(booking) {
  const timeStr = new Date(booking.start_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', timeZone: booking.timezone || 'America/Phoenix' });
  const firstName = booking.name.split(' ')[0];
    ...
    <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 560px; margin: 0 auto; color: #1a1a2e;">
      <div style="padding: 40px 32px; background: #ffffff; border-radius: 16px; border: 1px solid #e8e8f0;">
        ...
        <p style="color: #6b6b80; font-size: 15px; line-height: 1.6; margin: 0;">
          Your strategy session starts at <strong>${timeStr} ${getTzAbbr(booking)}</strong>.${booking.zoom_link ? '' : " We'll call the number you provided."}
        </p>
...
async function sendReminder10m(booking) {
  const timeStr = new Date(booking.start_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', timeZone: booking.timezone || 'America/Phoenix' });
  const firstName = booking.name.split(' ')[0];

  const html = `
    <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 560px; margin: 0 auto; color: #1a1a2e;">
      <div style="padding: 32px; background: #ffffff; border-radius: 16px; border: 1px solid #e8e8f0;">
        <div style="text-align: center; margin-bottom: 24px;">
          <h1 style="font-size: 14px; letter-spacing: 3px; color: #c9a46c; margin: 0; font-weight: 700;">VELOSLUXE</h1>
        </div>

        <h2 style="font-size: 22px; margin: 0 0 8px; color: #1a1a2e; font-weight: 700; text-align: center;">Starting in 10 minutes!</h2>
        <p style="color: #6b6b80; font-size: 15px; line-height: 1.6; margin: 0; text-align: center;">
          ${firstName}, your strategy call is at <strong>${timeStr} ${getTzAbbr(booking)}</strong>.
        </p>

        ${booking.zoom_link ? `
        <div style="text-align: center; margin: 24px 0;">
          <a href="${booking.zoom_link}" style="display: inline-block; padding: 16px 48px; background: #2D8CFF; color: #ffffff; border-radius: 100px; text-decoration: none; font-weight: 700; font-size: 16px;">
            Join Zoom Now
          </a>
        </div>
        ` : `
        <p style="color: #6b6b80; font-size: 14px; text-align: center; margin-top: 12px;">
  ...
  return send(booking.email, `Starting in 10 minutes — ${booking.zoom_link ? 'join Zoom now' : 'get ready'}!`, html);
}
