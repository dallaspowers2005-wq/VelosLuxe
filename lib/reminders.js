...
function scheduleReminders(booking) {
  ...
  const repPhone = process.env.GOOGLE_CAL_REP_PHONE;
  ...
  for (const [minutesBefore, prospectMsg, repMsg] of schedule) {
    ...
    if (sendAt > new Date()) {
      ...
      // Rep reminder
      if (repPhone) {
        _db.insertReminder(booking.id, repPhone, repMsg, sendAt.toISOString());
      }
    }
  }
}
...
async function sendConfirmation(booking) {
  ...
  if (repPhone) {
    await sendSMS(
      repPhone,
      `New booking: ${booking.name} (${booking.phone}${booking.email ? ', ' + booking.email : ''}) booked a strategy call for ${timeStr}.`
    );
  }
}
